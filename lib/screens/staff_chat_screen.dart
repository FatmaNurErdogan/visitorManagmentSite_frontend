...
/// Personelin (host ya da admin) bir ziyaretle ilgili ziyaretçiyle
/// mesajlaştığı ekran — VisitorChatScreen'in personel karşılığı. Ziyaretçi
/// bir accessToken ile giriyor, personel ise normal oturum (Bearer) ile;
/// backend tarafı: /api/mobile/visits/[id]/messages (visitorSite reposunda).
/// Sohbet, ziyaret talebi gönderilir gönderilmez (PENDING) zaten açık.
class StaffChatScreen extends StatefulWidget {
  const StaffChatScreen({super.key, required this.visit});
...
class _StaffChatScreenState extends State<StaffChatScreen> {
  ...
  String get _path => '/visits/${widget.visit.id}/messages';
  ...
  Future<void> _poll() async {
    try {
      ...
      if (!mounted) return;
      // 404 = ziyaret yok / bu sohbete erişimin yok — kalıcı, tekrar
      // denemenin anlamı yok. 409 (sohbet henüz/artık açık değil) durumu
      // ziyaretin durumuna göre değişebileceğinden polling'e devam ediyoruz.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.visit.visitor.name),
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isSelf = message.senderType == SenderType.staff;
